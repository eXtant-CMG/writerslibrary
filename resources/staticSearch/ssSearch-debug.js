/**
 * LOCAL PATCHES — do not overwrite with a fresh staticSearch download
 * without reapplying these changes.
 *
 * Bibundina project patches to ssSearch-debug.js
 * Applied: 2026-06-05
 *
 * 1. DOUBLE ENCODING FIX — setQueryString()
 *    The original code called encodeURI() on a string that already contained
 *    encodeURIComponent()-encoded values, causing double encoding of non-ASCII
 *    characters in the browser URL bar (e.g. %25C3%25A7 instead of %C3%A7).
 *    Fix: replaced encodeURI(search.join('&')) with search.join('&').
 *
 * 2. TRANSLITERATE FUNCTION — added before class StaticSearch
 *    Python's http.server decodes percent-encoded Unicode characters in URLs
 *    before looking up files on disk, making it impossible to serve files with
 *    percent-encoded Unicode names reliably. The XQuery indexer (search-index.xqm)
 *    therefore transliterates accented characters to ASCII equivalents in all
 *    stem values and filenames (e.g. français → francais). This JS function
 *    applies the same transliteration at search time so that stem lookups match
 *    the index. The character map must remain identical to si:transliterate()
 *    in search-index.xqm.
 *
 * 3. STEM FILENAME TRANSLITERATION — populateIndexes()
 *    Applied transliterate() to stemsToFind[i] when constructing the fetch URL,
 *    so that the requested filename matches the transliterated filename on disk.
 *
 * 4. INDEX KEY TRANSLITERATION — populateIndexes()
 *    Applied transliterate().toLowerCase() to this.terms[i].stem in the
 *    hasOwnProperty() cache check, so that repeated searches in the same
 *    session correctly find cached index entries stored under transliterated keys.
 *
 * 5. STEMFOUND KEY TRANSLITERATION — stemFound()
 *    Applied transliterate().toLowerCase() to data.stem when storing retrieved
 *    index data, so that the index is always keyed on the transliterated lowercase
 *    stem, matching both the filename and the processResults lookup.
 *
 * 6. PROCESSRESULTS STEM NORMALISATION — processResults()
 *    Added a loop at the start of processResults() that applies
 *    transliterate().toLowerCase() to every this.terms[i].stem before any
 *    index lookups are performed. This ensures that all internal result
 *    processing (processPhrases, processMustContains, processMayContains,
 *    processMustNotContains) uses transliterated stems consistent with the
 *    index keys.
 *
 * 7. WILDCARD RE-STEMMING FIX — wildcard expansion in doSearch()
 *    The original code re-stemmed words matched from ssWordString via
 *    this.stemmer.stem(m[1].toLowerCase()), but ssWordString already contains
 *    stems. Re-stemming produced incorrect values (e.g. francais → francai).
 *    Fix: replaced with m[1].toLowerCase() to use the matched stem directly.
 *
 * 8. WILDCARD TERM TYPE FIX — wildcard expansion in doSearch()
 *    The original code pushed wildcard-expanded terms as type PHRASE when
 *    allowPhrasal is true. Phrase matching requires the exact term to appear
 *    in the context snippet, which fails for wildcard results because the
 *    context window may not contain the matched word visibly. Fix: wildcard-
 *    expanded terms are always pushed as type MAY_CONTAIN regardless of
 *    allowPhrasal.
 */
...
//must match transliterate function in search-index.xqm
function transliterate(s) {
    if (!s) return '';
    const from = "àáâãäåçèéêëìíîïñòóôõöùúûüýÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖÙÚÛÜÝ";
    const to   = "aaaaaaceeeeiiiinooooouuuuyAAAAAACEEEEIIIINOOOOOUUUUY";
    return s.split('').map(function(c) {
        const i = from.indexOf(c);
        return i >= 0 ? to.charAt(i) : c;
    }).join('');
}
/*             StaticSearch.js             */
/* Authors: Martin Holmes and Joey Takeda. */
...
//We need a self pointer because this will go out of scope.
  ...
  //Now check whether we already have an index entry for this stem
        if (!this.index.hasOwnProperty(transliterate(this.terms[i].stem).toLowerCase())){
  //If not, add it to the array of stems we want to retrieve.
          stemsToFind.push(this.terms[i].stem);
        }
      }
      console.log('stemsToFind:', stemsToFind);
...
//assuming it's there.
          console.log('Looking up stem: [' + transliterate(stemsToFind[i]) + ']');
          promises[promises.length] = fetch(self.jsonDirectory + 'stems/' + transliterate(stemsToFind[i]) + this.versionString + '.json', this.fetchHeaders)
//If we get a response, and it looks good
              .then(function(response){
                console.log('Fetch status for ' + transliterate(stemsToFind[i]) + ': ' + response.status + ' ' + response.headers.get('content-type'));
...
/**
  ...
  */
  stemFound(data){
    try{
      console.log('stemFound data:', data);
      const key = transliterate(data.stem).toLowerCase();
      this.index[key] = data;
    }
    catch(e){
      console.log('ERROR: ' + e.message);
...
/**
  ...
  */
  processResults(){
    try{
      // Transliterate all stems to match the index keys
      for (let i = 0; i < this.terms.length; i++){
        this.terms[i].stem = transliterate(this.terms[i].stem).toLowerCase();
      }
