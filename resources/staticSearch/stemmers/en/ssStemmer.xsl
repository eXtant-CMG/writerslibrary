<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ss="http://hcmc.uvic.ca/ns/ssStemmer" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:math="http://www.w3.org/2005/xpath-functions/math" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" exclude-result-prefixes="#all" version="3.0">
  ...
  <xsl:variable name="step2Seq" as="xs:string+" select="(     'ousness:ous',     'iveness:ive',     'fulness:ful',     'ization:ize',     'ational:ate',     'biliti:ble',     'tional:tion',     'lessli:less',     'ousli:ous',     'fulli:ful',     'iviti:ive',     'entli:ent',     'alism:al',     'aliti:al',     'enci:ence',     'anci:ance',     'abli:able',     'izer:ize',     'ation:ate',     'ator:ate',     'alli:al',     'bli:ble',     'l|ogi:og',     concat($liEnding, '|li:')     )"/>
  ...
  <xsl:variable name="step3Seq" as="xs:string+" select="(     '1|ational:ate',     '1|tional:tion',     '1|alize:al',     '1|icate:ic',     '1|iciti:ic',     '2|ative:',     '1|ical:ic',     '1|ness:',     '1|ful:'     )"/>
  ...
  <xsl:variable name="step4Seq" as="xs:string+" select="(     'ement',     'ment',     'ance',     'ence',     'able',     'ible',     'ant',     'ent',     'ism',     'ate',     'iti',     'ous',     'ive',     'ize',     '[st]|ion',     'al',     'er',     'ic'     )"/>
  ...
  <xsl:variable name="exceptions" as="xs:string+" select="(     'skis:ski',     'skies:sky',     'dying:die',     'lying:lie',     'tying:tie',     'idly:idl',     'gently:gentl',     'ugly:ugli',     'early:earli',     'only:onli',     'singly:singl',     'sky:sky',     'news:news',     'howe:howe',     'atlas:atlas',     'cosmos:cosmos',     'bias:bias',     'andes:andes'     )"/>
  ...
  <xsl:variable name="exceptionsTokens" as="xs:string+" select="for $e in $exceptions return substring-before($e, ':')"/>
  ...
  <xsl:variable name="exceptionsStems" as="xs:string+" select="for $e in $exceptions return substring-after($e, ':')"/>
  ...
  <xsl:variable name="step1aExceptions" as="xs:string+" select="(   'inning', 'outing', 'canning', 'herring', 'earring', 'proceed', 'exceed', 'succeed'   )"/>
        ...
        <xsl:otherwise>
            <xsl:choose>
              ...
              <xsl:when test="matches($step1a, $step1b2Rex)">
                ...
                <xsl:value-of select="if (matches($step1b2a, '((at)|(bl)|(iz))$')) then concat($step1b2a, 'e')                    else if (matches($step1b2a, concat($dbl, '$'))) then                    substring($step1b2a, 1, string-length($step1b2a) - 1)                    else if (ss:wordIsShort($step1b2a, $R1)) then concat($step1b2a, 'e')                   else $step1b2a"/>
              </xsl:when>
...
<!--            <xsl:message><xsl:value-of select="concat('Matched ', $token, ' against ', $rex)"/></xsl:message>-->
  ...
  <xsl:function name="ss:getR1AndR2" as="item()+">
    <xsl:param name="token" as="xs:string"/>
    <xsl:variable name="R1" as="xs:string" select="if (matches($token, $R1ExceptRex))                                                     then replace($token, $R1ExceptRex, '$2')                                                     else if (matches($token, $R1R2Rex)) then                                                     replace($token, $R1R2Rex, '$1') else                                                    ''"/>
...
<!--    The second regex matches at the beginning, and we just need to know if it's only a
  ...
  <xsl:function name="ss:runTests" as="item()+">
    ...
    <xsl:variable name="fullTokenSet" select="     if (unparsed-text-available('voc.txt'))      then tokenize(unparsed-text('voc.txt'), '[\n\s]+')     else tokenize(unparsed-text($fullTokenSetUrl), '[\n\s]+')"/>
    <xsl:variable name="fullStemSet" select="       if (unparsed-text-available('output.txt'))        then tokenize(unparsed-text('output.txt'), '[\n\s]+')       else tokenize(unparsed-text($fullStemSetUrl), '[\n\s]+')"/>  
